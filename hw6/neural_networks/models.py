...
class NeuralNetwork(ABC):
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        for i in range(self.n_layers):
            currentLayer = self.layers[self.n_layers - 1 - i]
            dLdY = currentLayer.backward(dLdY)
    ...
    def test(self, dataset: Dataset, save_predictions: bool = False) -> Dict[str, List[np.ndarray]]:
    ...
    def predict(self, X: np.ndarray, Y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        loss = self.backward(Y, prediction)
