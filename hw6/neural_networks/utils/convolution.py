...
def pad_dilate2d(dLdZ, kernel_height, kernel_width, stride):
    n, h, w, d = dLdZ.shape
    h_new = h + (h - 1) * (stride - 1)
    w_new = w + (w - 1) * (stride - 1)
    pad_h = kernel_height - 1
    pad_w = kernel_width - 1
    dLdZ_new = np.zeros((n, h_new + 2 * pad_h + 2, w_new + 2 * pad_w + 2,  d))
    for i in range(h):
        for j in range(w):
            dLdZ_new[:, pad_h + i * stride, pad_w + j * stride, :] = dLdZ[:, i, j, :]
